...
private slots:
  ...
  void init() {
    ...
    QSignalSpy waitForFeedListPopulated(m_context.get(),
                                        &Context::feedListPopulated);
  ...
  void testAllItemsFeedContainsAllItems() {
    ...
    QObject::connect(
        getArticles, &BaseFuture::finished, this,
        [getArticles, &aafItems] { aafItems = getArticles->result(); });
  ...
  void testAllItemsFeedImplementsUnreadFilter() {
    ...
    QVector<ArticleRef> unreadItems = {m_mockFeed1->m_articles[1],
                                       m_mockFeed2->m_articles[0]};
  ...
  void testAllItemsFeedEmitsArticleAddedSignals() {
    QSignalSpy waitArticleAdded(m_allItemsFeed.get(),
                                &FeedCore::Feed::articleAdded);
